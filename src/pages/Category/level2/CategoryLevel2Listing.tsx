...
const CategoryLevel2Listing = () => {

    let breadCrumbPath = [
        { name: 'Home', url: `/`, active: '' },
        { name: 'Sub-Category List', url: ``, active: 'not-allowed' },
    ]

    const limit = 10;
    const navigate = useNavigate();
    const location = useLocation();
    const match = useMatch("/category/level-2/:page")
    ...
    const initialise = async (search: string, pagination: number) => {
        try {
            setLoading(true)
            const apiRes = await henceforthApi.Category.listSubCategory(search, pagination - 1, 10)
            setState(apiRes)
        } catch (error: any) {
    ...
    const onChangePagination = (page: number) => {
        navigate({ pathname: `/category/level-2/${page}`, search: urlSearchParams.toString() })
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    let apiRes = await henceforthApi.Category.subCategoryDelete(data)
    ...
    const exportData = async (startDate: number, endDate: number) => {
        try {
            const apiRes = await henceforthApi.Category.exportSubCategory(startDate, endDate)
    ...
    useEffect(() => {
        initialise(urlSearchParams.get("search") as string, Number(match?.params.page))
    }, [urlSearchParams.get("search"), match?.params.page, authState?.lang])
                                    ...
                                    <div className='d-flex gap-3 justify-content-md-end'>
                                        <div className='download-export-box'>
                                            ...
                                            <div className="export-button">
                                                <Link to="/category/level-2/add" className="btn btn-white"> <i className='fa fa-plus me-2'></i>Add</Link>
                                            </div>
                                        </div>
                                        {/* export  */}
                                        <div className='download-export-box'>
                                                        ...
                                                        {state?.data?.data?.map((res, index) =>
                                                            <tr key={res._id}>
                                                                ...
                                                                <td><div className="btn-group gap-2">
                                                                    ...
                                                                    <Link className="btn btn-white btn-sm" to={`/category/level-2/edit/${res.name}/${res?._id}`}> <i className="fa fa-pencil me-1"></i>Edit</Link>
...
export default CategoryLevel2Listing;