...
​
const InvoiceDetail = ({dataHolder}:any) => {
    return (
        <Fragment>
            <View style={styles.container}>
                ...
                <View style={styles.w_30}>
                    {/* Order id  */}
                    <Text style={[{ fontFamily: 'bold', marginBottom: 5 }]}>Order ID: {dataHolder?.order_id?dataHolder?.order_id:'Not Available'}</Text>
                    <View style={[styles.container, { marginBottom: 5 }]}>
                        {/* Order date  */}
                        <Text style={[{ fontFamily: 'bold', marginRight: 3 }]}>Order Date:</Text>
                        <Text>{dataHolder?.created_at?moment(+dataHolder?.created_at).format('DD-MM-YYYY'):"Not Available"}</Text>
                    </View>
                    {/* Invoice date  */}
                    <View style={[styles.container, { marginBottom: 5 }]}>
                ...
                <View style={styles.w_30}>
                    ...
                    <Text style={[{ fontFamily: 'bold', marginBottom: 4 }]}>{dataHolder?.user_id?.name?dataHolder?.user_id?.name:'Not Available'}</Text>
                    <Text  style={[{ width: '80%'}]}>{dataHolder?.address_id?.full_address?dataHolder?.address_id?.full_address:'Not Avaialble'}</Text>
                    ...
                    <Text> {dataHolder?.user_id?.phone_no?`Phone:${dataHolder?.user_id?.phone_no}`:'Not Available'}</Text>
                </View>
                {/* Ship To  */}
                <View style={styles.w_20}>
                    <Text style={[{ fontFamily: 'bold' }]}>Ship To</Text>
                    <Text style={[{ fontFamily: 'bold', marginBottom: 4 }]}>{dataHolder?.user_id?.name?dataHolder?.user_id?.name:'Not Available'}</Text>
                    <Text>{dataHolder?.address_id?.full_address?dataHolder?.address_id?.full_address:'Not Avaialble'}</Text>
